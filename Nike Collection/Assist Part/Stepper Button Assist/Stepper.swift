...
class Stepper: UIControl {
    ...
    @objc @IBInspectable var stepValue: Double = 1
    ...
    @objc @IBInspectable var autorepeat = true
    
    /// If the value is integer, it is shown without floating point.
    @objc @IBInspectable var showIntegerIfDoubleIsInteger = true
    
    // Text on the left button. Be sure that it fits in the button. Defaults to "−".
    @objc @IBInspectable var leftButtonText: String = "−" {
        didSet {
    ...
    @objc @IBInspectable var rightButtonText: String = "+" {
        didSet {
    ...
    @objc @IBInspectable var labelTextColor: UIColor = UIColor.white {
        didSet {
    ...
    @objc @IBInspectable var labelBackgroundColor: UIColor = UIColor(red:0.26, green:0.6, blue:0.87, alpha:1) {
        didSet {
    ...
    @objc var labelFont = UIFont(name: "AvenirNext-Bold", size: 25.0)! {
        didSet {
            label.font = labelFont
        }
    }
    // Corner radius of the middle label. Defaults to 0.
    @objc @IBInspectable var labelCornerRadius: CGFloat = 0 {
        didSet {
    ...
    @objc @IBInspectable var cornerRadius: CGFloat = 4.0 {
        didSet {
    ...
    @objc @IBInspectable var borderWidth: CGFloat = 0.0 {
        didSet {
    ...
    @objc @IBInspectable var borderColor: UIColor = UIColor.clear {
        didSet {
    ...
    @objc @IBInspectable var limitHitAnimationColor = UIColor(red:0.26, green:0.6, blue:0.87, alpha:1)
    ...
    lazy var label: UILabel = {
        ...
        let panRecognizer = UIPanGestureRecognizer(target: self, action: #selector(Stepper.handlePan))
    ...
    func setup() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(Stepper.reset), name: NSNotification.Name.UIApplicationWillResignActive, object: nil)
    }
...
extension Stepper {
    ...
    func scheduleTimer() {
        ...
        timer = Timer.scheduledTimer(timeInterval: timerInterval, target: self, selector: #selector(Stepper.handleTimerFire), userInfo: nil, repeats: true)
    }
